...
def generate(filename, override_system_clock_frequency=None, manual_overlays=None):
    ...
    def node_sort_key(node):
        # Sort CPUs first by their unit address, then other devices by their address and compatible field
        compat = get_node_prop(node, 'compatible')[0] if 'compatible' in node.props else ''
        is_cpu = get_node_prop(node, 'device_type') == 'cpu' or node.name.startswith('cpu@') or node.name == 'cpu'
        try:
            unit_addr = int(node.unit_addr, 16)
        except ValueError:
            unit_addr = 0
    ...
    for node in nodes:
        ...
        if model == 'CPU.RiscV32' or model == 'CPU.Ri5cy':  # We use CPU.RiscV32 as a generic model for all RV CPUs and fix it up here
            ...
            if overlays & {'litex,vexriscv', 'kosagi,fomu'}:
                ...
                empty_time_provider_compats = {
                    ...
                    "sensry,sy1xx", "nordic,vpr", "adi,max32-rv32", "cdac,vega-et1031"
                }
        ...
        if model in ("CPU.ARMv8A", "CPU.ARMv8R", "CPU.ARMv7A", "CPU.ARMv7R"):
            ...
            and (main_compatible is None or not any(element in ["beagle,beaglebone-ai64",
                                                                "beagle,beagley-ai",
                                                                "grinn,am335x-chiliboard",
                                                                "brcm,bcm2837"] for element in
                                                    main_compatible)) \
